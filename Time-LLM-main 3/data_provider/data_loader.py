...
class StockDataset(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.scale:
            ...
            data = df_data.values
        if self.root_path ==  '../FNSPID_dataset/fin/':
    ...
    def __getitem__(self, index):
        ...
        if self.root_path == '../FNSPID_dataset/fin/':
            seq = (self.df_stamp['Date'][s_begin:s_end].dt.tz_localize(None) - pd.Timestamp("1970-01-01")) // pd.Timedelta('1s') # unix time stamp
        else:
            seq = (self.df_stamp['date'][s_begin:s_end].dt.tz_localize(None) - pd.Timestamp("1970-01-01")) // pd.Timedelta('1s') # unix time stamp

        seq = seq.values.astype(np.int64)
