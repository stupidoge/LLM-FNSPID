...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        self.linear_contrast = nn.Linear(32, 1000)
        self.linear_contrast_de = nn.Linear(1000,32)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            dec_out, contrast_loss = self.forecast(x_enc, x_mark_enc, x_dec, x_mark_dec)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        contrast_source_embeddings = source_embeddings # source_embedding:(1000,768)
        contrast_enc_out = enc_out.reshape(-1,32)  # enc_out:(12,64,32)
        contrast_enc_out = self.linear_contrast(contrast_enc_out).permute(1,0)
        contrast_loss_fn = ContrastiveLoss()
        contrast_loss = contrast_loss_fn(contrast_enc_out, contrast_source_embeddings)
        # project back
        enc_out = self.linear_contrast_de(contrast_enc_out.permute(1,0)).reshape(12,64,32)
        source_embeddings = contrast_source_embeddings
