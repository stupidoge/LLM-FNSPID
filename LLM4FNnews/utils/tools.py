...
class EarlyStoppingFin:
    ...
    def __call__(self, val_loss, test_mse, test_mae, model, path):
        score = -val_loss
        if self.best_score is None:
            self.best_score = score
            if self.save_mode:
                self.save_checkpoint(val_loss, test_mse, test_mae, model, path)
            ...
            if self.save_mode:
                self.save_checkpoint(val_loss, test_mse, test_mae, model, path)
            self.counter = 0
    def save_checkpoint(self, val_loss, test_mse, test_mae, model, path):
        if self.verbose:
        ...
        self.test_mse_min = test_mse
        self.test_mae_min = test_mae
...
def vali_pretrain(args, accelerator, model, vali_loader, criterion, mae_metric):
   total_loss = []
   total_mae_loss = []
   model.eval()
   with torch.no_grad():
       for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_seq) in tqdm(enumerate(vali_loader)):
           ...
           if args.use_amp:
               ...
               if args.output_attention:
                   outputs, _ = model(batch_x, batch_x_mark, dec_inp, batch_y_mark, batch_seq)[0]
               else:
                   outputs, _ = model(batch_x, batch_x_mark, dec_inp, batch_y_mark, batch_seq)
           # self.accelerator.wait_for_everyone()
           f_dim = -1 if args.features == 'MS' else 0
           outputs = outputs[:, -args.pred_len:, f_dim:]
           batch_y = batch_y[:, -args.pred_len:, f_dim:].to(accelerator.device)
   ...
   model.train()
   return total_loss, total_mae_loss
def vali_contrast(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    total_loss = []
    total_mae_loss = []
    model.eval()
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
            ...
            true = batch_y.detach()
            # w = 0.01
            # loss = (1-w) * criterion(pred, true) + w * contrast_loss
...
def vali_contrast_fin(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    total_loss = []
    total_mae_loss = []
    model.eval()
    with torch.no_grad():
